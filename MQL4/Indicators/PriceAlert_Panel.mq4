...
void CPriceAlertPanel::RefreshValues()
{
    ...
    else if (sets.AlertOnPriceSetting == PreviousClose) price = iClose(Symbol(), ClosePriceTimeframe, 1);
    if (price == 0) return; // Protection against connection loss.
    ...
    if (sets.ExactlyEnabled && sets.PriceExactly > 0)
    {
        ...
        {
            double prev_close = iClose(Symbol(), ClosePriceTimeframe, 1);
            if (prev_close != 0 && MathAbs(prev_close - sets.PriceExactly) < tolerance)
            {
                exactly_hit = true;
                exactly_price = prev_close;
            }
        }

        if (exactly_hit && exactly_price != 0) // Protection against connection loss.
        {
...
void FireAlert(const ENUM_ALERT_TYPE type, const double level, const double price, const string sound_file)
{
    ...
    if (sets.AlertNative) Alert(Symbol(), ", ", period_str, ": ", alert_phrase, " - ", level_str, ".");
